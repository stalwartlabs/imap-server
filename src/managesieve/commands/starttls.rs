...
use crate::{
    core::writer,
    managesieve::{client::Session, StatusResponse},
};

impl Session {
    pub async fn handle_starttls(&mut self) -> Result<Option<WriteHalf<TcpStream>>, ()> {
        self.write_bytes(StatusResponse::ok("Begin TLS negotiation now").into_bytes())
            .await?;
